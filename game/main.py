...
my_game = Board(rows=ROWNUM, cols=COLNUM, winning_condition=winning_condition, XO=XO)
AgentRandom = AgentRandom(my_game)
...
replay_button = Button(970, 575, replay_img, replay_img, 0.8)
exit_button = Button(970, 485, exit_img, exit_img, 0.8)
...
def draw(this_game: Board.Board, this_screen):
    logo()
...
def main():
    ...
    # ------------ai vs p button------------------------------------------------
            if aivp_btn.draw(Screen):
                ...
                agent = AgentRandom(max_depth=my_game.hard_ai,
                                    XO=my_game.get_current_XO_for_AI())
